...
import { type CreateDebtorParams, createDebtor } from '@/lib/db/debtors';
import {
...
describe('Debts Database Operations', () => {
	...
	beforeEach(async () => {
		...
		testDebtorEmail = debtor.email ?? 'john.doe@example.com';
	});
	...
	describe('createDebt', () => {
		it('should create debt with valid data', async () => {
			...
			expect(Number(result.amountPaid)).toBe(250.0);
		...
		it('should handle large amounts', async () => {
			const params: CreateDebtParams = {
				...
				totalOwed: 99_999_999.99, // Max for DECIMAL(10,2)
			};

			const result = await createDebt(params);
			expect(Number(result.totalOwed)).toBe(99_999_999.99);
		});
	});
	...
	describe('getDebtById', () => {
		it('should return debt when found', async () => {
			...
			expect(Number(result?.totalOwed)).toBe(500.0);
		});
	...
	describe('getDebtsByDebtorEmail', () => {
		it('should return debts for a debtor email', async () => {
			...
			expect(result.map((d) => d.originalCreditor)).toContain(
				'Second Creditor'
			);
		});
		...
		it('should handle multiple debtors with same email (testing TODO comment)', async () => {
			...
			await createDebtor(debtor2Params);
	...
	describe('getDebtByDebtorId', () => {
		...
		it('should return null when no debt found for debtor', async () => {
			const result = await getDebtByDebtorId(
	...
	describe('paidDebt', () => {
		...
		it('should update debt with payment amount and default status', async () => {
			...
			expect(Number(updated?.amountPaid)).toBe(500.0);
		...
		it('should update debt with custom status', async () => {
			...
			const updated = await getDebtById(testDebtId);
			expect(Number(updated?.amountPaid)).toBe(250.0);
		...
		it('should handle full payment', async () => {
			...
			const updated = await getDebtById(testDebtId);
			expect(Number(updated?.amountPaid)).toBe(1000.0);
		...
		it('should handle overpayment', async () => {
			await paidDebt({
				id: testDebtId,
				amountPaid: 1500.0, // More than totalOwed
				status: 'overpaid',
			});

			const updated = await getDebtById(testDebtId);
			expect(Number(updated?.totalOwed)).toBe(1000.0);
			expect(Number(updated?.amountPaid)).toBe(1500.0);
	...
	describe('Edge cases and data integrity', () => {
      ...
      `,
		...
		it('should handle special characters in creditor names', async () => {
			...
			const result = await createDebt(params);
			expect(result.originalCreditor).toBe(
				"O'Reilly & Associates Inc. - Collection Dept."
			);
		});
