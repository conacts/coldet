...
describe('Emails Database Operations', () => {
	...
	describe('createEmail', () => {
		...
		it('should handle Unicode characters in subject and content', async () => {
			const emailParams = {
				...
				content:
					'Hola, necesito información sobre mi cuenta. ¿Pueden ayudarme? 🙏',
			};
	...
	describe('Edge cases and data integrity', () => {
      ...
      `,
			...
			const row = result.rows[0] as { first_name: string; last_name: string; original_creditor: string; thread_subject: string };
			expect(row.first_name).toBe('John');
			expect(row.last_name).toBe('Doe');
			expect(row.original_creditor).toBe('Test Creditor');
			expect(row.thread_subject).toBe('Test Thread Subject');
		});

		it('should handle very long content', async () => {
			const longContent = 'A'.repeat(10_000); // Test large TEXT field
			const emailParams = {
				organizationId: testOrganizationId,
				debtId: testDebtId,
				threadId: testThreadId,
		...
		it('should handle special characters in email addresses', async () => {
			...
			const result = await createEmail(emailParams);
			expect(result.fromEmailAddress).toBe(
				'user+tag@sub-domain.example-site.co.uk'
			);
		});

		it('should handle HTML content in email body', async () => {
			const htmlContent =
				'<html><body><p>Hello <strong>John</strong>,</p><p>Your balance is <em>$1,000</em>.</p></body></html>';
			const emailParams = {
				organizationId: testOrganizationId,
				debtId: testDebtId,
				threadId: testThreadId,
