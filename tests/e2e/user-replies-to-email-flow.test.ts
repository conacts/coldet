...
describe('User Replies to Email Flow E2E Test', () => {
	...
	beforeEach(async () => {
		...
		mockedRender.mockResolvedValue(
			'<html><body>AI Response Email</body></html>'
		);
	...
	describe('Complete User Reply Flow', () => {
		it('should handle a complete user reply workflow', async () => {
			...
			const initialThread = await createEmailThread(
				testOrganizationId,
				debtor.id,
			...
			const replyNotification: SesSnsNotification = {
				...
				mail: {
					...
					headers: [
						...
						{
							name: 'In-Reply-To',
							value: '<initial-outbound-msg-123@coldets.com>',
						},
						{
							name: 'References',
							value: '<initial-outbound-msg-123@coldets.com>',
				...
				content:
					'Hi, I received your email about my outstanding balance. I would like to discuss payment options. Can you please call me at 555-123-4567? Thank you.',
			};
      ...
      `,
			...
			expect(inboundEmail.content).toBe(
				'Hi, I received your email about my outstanding balance. I would like to discuss payment options. Can you please call me at 555-123-4567? Thank you.'
			);
      ...
      `,
			...
			expect(allThreadEmails.rows[0].message_id).toBe(
				'initial-outbound-msg-123'
			);
			expect(allThreadEmails.rows[1].message_id).toBe('reply-message-id-456');
			expect(allThreadEmails.rows[2].message_id).toBe(
				'ai-response-message-id-12345'
			);
		});
		...
		it('should handle reply to existing thread using In-Reply-To header', async () => {
			...
			const replyNotification: SesSnsNotification = {
				...
				content:
					'I can pay $500 now and $1000 in 30 days. Please confirm if this is acceptable.',
			};
      ...
      `,
			...
			expect(threadEmails.rows[1].message_id).toBe(
				'reply-to-existing-thread-123'
			);
		...
		it('should create new thread when no existing thread found', async () => {
			...
			const newThreadNotification: SesSnsNotification = {
				...
				content:
					'I have a question about my student loan balance. Can you provide more details?',
			};
      ...
      `,
			...
			expect(inboundEmail.rows[0].content).toBe(
				'I have a question about my student loan balance. Can you provide more details?'
			);
		});
	});
	...
	describe('Error Handling in Reply Flow', () => {
		it('should handle unknown sender email', async () => {
			...
			await expect(
				handleReceivedEmail(unknownSenderNotification)
			).rejects.toThrow('No debtor found for sender email');
		});
		...
		it('should handle missing email content', async () => {
			...
			await expect(handleReceivedEmail(noContentNotification)).rejects.toThrow(
				'Email content is required but was not provided'
			);
		});
		...
		it('should handle AI response generation failure', async () => {
			...
			mockedGenerateResponseEmail.mockRejectedValue(
				new Error('AI service unavailable')
			);

			await expect(handleReceivedEmail(aiFailureNotification)).rejects.toThrow(
	...
	describe('Data Validation in Reply Flow', () => {
		it('should handle special characters in email content', async () => {
			...
			const unicodeNotification: SesSnsNotification = {
				...
				content:
					'Hola, necesito información sobre mi cuenta. ¿Pueden ayudarme con el pago? Gracias 🙏',
			};
      ...
      `,
			...
			expect(inboundEmail.rows).toHaveLength(1);
			expect(inboundEmail.rows[0].subject).toBe(
